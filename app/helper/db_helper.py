...
class DbHelper:
    ...
    def insert_search_results(
        self, media_items: list, title=None, ident_flag=True
    ):
        """
        将返回信息插入数据库
        """
        ...
        for media_item in media_items:
            ...
            data_list.append(
                SEARCHRESULTINFO(
                    ...
                    SEASON=(
                        media_item.get_season_string() if ident_flag else ""
                    ),
                    EPISODE=(
                        media_item.get_episode_string() if ident_flag else ""
                    ),
                    ES_STRING=(
                    ...
                    IMAGE=media_item.get_backdrop_image(
                        default=False, original=True
                    ),
    ...
    def get_search_result_by_id(self, dl_id):
        ...
        """
        return (
            self._db.query(SEARCHRESULTINFO)
            .filter(SEARCHRESULTINFO.ID == dl_id)
            .all()
        )
    ...
    def is_transfer_history_exists(
        self, source_path, source_filename, dest_path, dest_filename
    ):
        """
        查询识别转移记录
        """
    ...
    def update_transfer_history_date(
        self, source_path, source_filename, dest_path, dest_filename, date
    ):
        """
        更新历史转移记录时间
        """
    ...
    def insert_transfer_history(
        ...
        if self.is_transfer_history_exists(
            source_path, source_filename, dest_path, dest_filename
        ):
            # 更新历史转移记录的时间
            self.update_transfer_history_date(
                source_path, source_filename, dest_path, dest_filename, timestr
            )
            return
        dest = dest or ""
        self._db.insert(
    ...
    def get_transfer_info_by_id(self, logid):
        ...
        return (
            self._db.query(TRANSFERHISTORY)
            .filter(TRANSFERHISTORY.ID == int(logid))
            .first()
        )
    ...
    def get_transfer_info_by(self, tmdbid, season=None, season_episode=None):
        ...
        if tmdbid and not season and not season_episode:
            return (
                self._db.query(TRANSFERHISTORY)
                .filter(TRANSFERHISTORY.TMDBID == int(tmdbid))
                .all()
            )
        # 电视剧某季
    ...
    def get_transfer_unknown_paths(self):
        ...
        return (
            ...
            .filter(TRANSFERUNKNOWN.STATE == "N")
            .all()
        )
    ...
    def get_unknown_info_by_id(self, tid):
        ...
        return (
            self._db.query(TRANSFERUNKNOWN)
            .filter(TRANSFERUNKNOWN.ID == int(tid))
            .first()
        )
    ...
    def get_transfer_unknown_by_path(self, path):
        ...
        return (
            self._db.query(TRANSFERUNKNOWN)
            .filter(TRANSFERUNKNOWN.PATH == path)
            .all()
        )
    ...
    def is_need_insert_transfer_unknown(self, path):
        ...
        if unknowns:
            ...
            if is_all_proceed:
                is_transfer_history_exists = (
                    self.is_transfer_history_exists_by_source_full_path(path)
                )
    ...
    def insert_transfer_unknown(self, path, dest, rmt_mode):
        ...
        if self.is_transfer_unknown_exists(path):
            ...
            self._db.insert(
                TRANSFERUNKNOWN(
                    PATH=path, DEST=dest, STATE="N", MODE=str(rmt_mode.value)
                )
            )
    ...
    def delete_transfer_blacklist(self, path):
        ...
        删除黑名单记录
        """
        self._db.query(TRANSFERBLACKLIST).filter(
            TRANSFERBLACKLIST.PATH == str(path)
        ).delete()
        self._db.query(SYNCHISTORY).filter(
            SYNCHISTORY.PATH == str(path)
        ).delete()
    ...
    def get_site_by_id(self, tid):
        ...
        """
        return (
            self._db.query(CONFIGSITE).filter(CONFIGSITE.ID == int(tid)).all()
        )
    ...
    def update_config_site(
        self, tid, name, site_pri, rssurl, signurl, cookie, note, rss_uses
    ):
        """
        更新站点信息
        """
    ...
    def update_site_cookie_ua(self, tid, cookie, ua=None):
        ...
        if not tid:
            return
        rec = (
            self._db.query(CONFIGSITE)
            .filter(CONFIGSITE.ID == int(tid))
            .first()
        )
        if rec.NOTE:
    ...
    def get_config_filter_group(self, gid=None):
        ...
        if gid:
            return (
                self._db.query(CONFIGFILTERGROUP)
                .filter(CONFIGFILTERGROUP.ID == int(gid))
                .all()
            )
    ...
    def get_rss_movie_id(self, title, year=None, tmdbid=None):
        ...
        if tmdbid:
            ret = (
                self._db.query(RSSMOVIES.ID)
                .filter(RSSMOVIES.TMDBID == str(tmdbid))
                .first()
            )
            if ret:
                return ret[0]
        if not year:
            items = (
                self._db.query(RSSMOVIES).filter(RSSMOVIES.NAME == title).all()
            )
        else:
            items = (
                self._db.query(RSSMOVIES)
    ...
    def update_rss_movie_tmdb(
        self, rid, tmdbid, title, year, image, desc, note
    ):
        """
        更新订阅电影的部分信息
        """
    ...
    def update_rss_filter_order(self, rtype, rssid, res_order):
        ...
        if rtype == MediaType.MOVIE:
            self._db.query(RSSMOVIES).filter(
                RSSMOVIES.ID == int(rssid)
            ).update({"FILTER_ORDER": res_order})
        else:
    ...
    def get_rss_overedition_order(self, rtype, rssid):
        ...
        if rtype == MediaType.MOVIE:
            res = (
                self._db.query(RSSMOVIES.FILTER_ORDER)
                .filter(RSSMOVIES.ID == int(rssid))
                .first()
            )
        else:
            res = (
                self._db.query(RSSTVS.FILTER_ORDER)
                .filter(RSSTVS.ID == int(rssid))
                .first()
            )
        if res and res[0]:
            return int(res[0])
    ...
    def update_rss_movie_state(
        self, title=None, year=None, rssid=None, state="R"
    ):
        """
        更新电影订阅状态
        """
    ...
    def get_rss_tvs(self, state=None, rssid=None):
        ...
        if rssid:
            ...
            if not state:
                ...
                return (
                    self._db.query(RSSTVS).filter(RSSTVS.STATE == state).all()
                )
    ...
    def update_rss_tv_tmdb(
        self, rid, tmdbid, title, year, total, lack, image, desc, note
    ):
        """
        更新订阅电影的TMDBID
        """
    ...
    def insert_rss_tv(
        ...
        if self.is_exists_rss_tv(
            media_info.title, media_info.year, season_str
        ):
            return 9
        self._db.insert(
            RSSTVS(
    ...
    def delete_rss_tv(self, title=None, season=None, rssid=None, tmdbid=None):
        ...
        if not title and not rssid:
            return
        if not rssid:
            rssid = self.get_rss_tv_id(
                title=title, tmdbid=tmdbid, season=season
            )
        if rssid:
    ...
    def is_exists_rss_tv_episodes(self, rid):
        ...
        if not rid:
            return False
        count = (
            self._db.query(RSSTVEPISODES)
            .filter(RSSTVEPISODES.RSSID == int(rid))
            .count()
        )
        if count > 0:
            return True
    ...
    def update_rss_tv_episodes(self, rid, episodes):
        ...
        if self.is_exists_rss_tv_episodes(rid):
            self._db.query(RSSTVEPISODES).filter(
                RSSTVEPISODES.RSSID == int(rid)
            ).update({"EPISODES": ",".join(episodes)})
        else:
            self._db.insert(
    ...
    def get_rss_tv_episodes(self, rid):
        ...
        if not rid:
            return []
        ret = (
            self._db.query(RSSTVEPISODES.EPISODES)
            .filter(RSSTVEPISODES.RSSID == rid)
            .first()
        )
        if ret:
    ...
    def update_rss_tv_state(
        self, title=None, year=None, season=None, rssid=None, state="R"
    ):
        """
        更新电视剧订阅状态
        """
    ...
    def get_users(self, uid=None, name=None):
        ...
        if uid:
            return (
                self._db.query(CONFIGUSERS)
                .filter(CONFIGUSERS.ID == uid)
                .first()
            )
        elif name:
            return (
                self._db.query(CONFIGUSERS)
                .filter(CONFIGUSERS.NAME == name)
                .first()
            )
    ...
    def update_site_user_statistics_site_name(self, new_name, old_name):
        ...
        self._db.query(SITEUSERINFOSTATS).filter(
            SITEUSERINFOSTATS.SITE == old_name
        ).update({"SITE": new_name})
    ...
    def is_exists_site_user_statistics(self, url):
        ...
        count = (
            ...
            .filter(SITEUSERINFOSTATS.URL == url)
            .count()
        )
        if count > 0:
            return True
    ...
    def is_exists_site_favicon(self, site):
        ...
        """
        count = (
            self._db.query(SITEFAVICON)
            .filter(SITEFAVICON.SITE == site)
            .count()
        )
        if count > 0:
            return True
    ...
    def update_site_seed_info_site_name(self, new_name, old_name):
        ...
        self._db.query(SITEUSERSEEDINGINFO).filter(
            SITEUSERSEEDINGINFO.SITE == old_name
        ).update({"SITE": new_name})
    ...
    def get_site_user_statistics(self, num=100, strict_urls=None):
        ...
        if strict_urls:
            ...
            return (
                ...
                .filter(
                    SITEUSERINFOSTATS.URL.in_(
                        tuple(strict_urls + ["__DUMMY__"])
                    )
                )
    ...
    def update_site_statistics_site_name(self, new_name, old_name):
        ...
        self._db.query(SITESTATISTICSHISTORY).filter(
            SITESTATISTICSHISTORY.SITE == old_name
        ).update({"SITE": new_name})
    ...
    def insert_site_statistics_history(self, site_user_infos: list):
        ...
        for site_user_info in site_user_infos:
            ...
            bonus = site_user_info.bonus
            url = site_user_info.site_url
            if not self.is_site_statistics_history_exists(
                date=date_now, url=url
            ):
                self._db.insert(
    ...
    def is_site_seeding_info_exist(self, url):
        ...
        """
        count = (
            self._db.query(SITEUSERSEEDINGINFO)
            .filter(SITEUSERSEEDINGINFO.URL == url)
            .count()
        )
        if count > 0:
            return True
    ...
    def get_site_statistics_recent_sites(
        self, days=7, end_day=None, strict_urls=None
    ):
        """
        查询近期上传下载量
        ...
        if date_ret and date_ret[0][0]:
            ...
            if strict_urls:
                subquery = (
                    ...
                    .group_by(
                        SITESTATISTICSHISTORY.SITE, SITESTATISTICSHISTORY.DATE
                    )
                    .subquery()
                )
            else:
    ...
    def is_exists_download_history(self, enclosure, downloader, download_id):
        ...
        if enclosure:
            count = (
                ...
                .filter(DOWNLOADHISTORY.ENCLOSURE == enclosure)
                .count()
            )
        else:
            count = (
    ...
    def insert_download_history(
        self, media_info, downloader, download_id, save_dir
    ):
        """
        新增下载历史
        """
    ...
    def get_download_history(self, date=None, hid=None, num=30, page=1):
        ...
        if hid:
            return (
                self._db.query(DOWNLOADHISTORY)
                .filter(DOWNLOADHISTORY.ID == int(hid))
                .all()
            )
        sub_query = (
            self._db.query(DOWNLOADHISTORY, func.max(DOWNLOADHISTORY.DATE))
        ...
        if date:
            return (
                ...
                .join(sub_query, and_(sub_query.c.ID == DOWNLOADHISTORY.ID))
    ...
    def get_download_history_by_title(self, title):
        ...
        return (
            self._db.query(DOWNLOADHISTORY)
            .filter(DOWNLOADHISTORY.TITLE == title)
            .all()
        )
    ...
    def get_brushtasks(self, brush_id=None):
        ...
        if brush_id:
            return (
                ...
                .filter(SITEBRUSHTASK.ID == int(brush_id))
                .first()
            )
        else:
            # 根据站点优先级排序
    ...
    def get_brushtask_totalsize(self, brush_id):
        ...
        ret = (
            self._db.query(
                func.sum(cast(SITEBRUSHTORRENTS.TORRENT_SIZE, Integer))
            )
            .filter(
                SITEBRUSHTORRENTS.TASK_ID == brush_id,
    ...
    def update_brushtask_state(self, state, tid=None):
        ...
        if tid:
            self._db.query(SITEBRUSHTASK).filter(
                SITEBRUSHTASK.ID == int(tid)
            ).update({"STATE": "Y" if state == "Y" else "N"})
        else:
    ...
    def add_brushtask_upload_count(
        self, brush_id, upload_size, download_size, remove_count
    ):
        """
        更新上传下载量和删除种子数
        """
    ...
    def insert_brushtask_torrent(
        self, brush_id, title, enclosure, downloader, download_id, size
    ):
        """
        增加刷流下载的种子信息
        """
    ...
    def get_brushtask_torrent_by_enclosure(self, enclosure):
        ...
        return (
            self._db.query(SITEBRUSHTORRENTS)
            .filter(SITEBRUSHTORRENTS.ENCLOSURE == enclosure)
            .first()
        )
    ...
    def add_filter_group(self, name, default="N"):
        ...
        if group_id:
            self._db.query(CONFIGFILTERGROUP).filter(
                CONFIGFILTERGROUP.ID == int(group_id)
            ).update({"IS_DEFAULT": default})
        else:
            self._db.insert(
                CONFIGFILTERGROUP(GROUP_NAME=name, IS_DEFAULT=default)
            )
    ...
    def set_default_filtergroup(self, groupid):
        ...
        """
        self._db.query(CONFIGFILTERGROUP).filter(
            CONFIGFILTERGROUP.ID == int(groupid)
        ).update({"IS_DEFAULT": "Y"})
        self._db.query(CONFIGFILTERGROUP).filter(
            CONFIGFILTERGROUP.ID != int(groupid)
        ).update({"IS_DEFAULT": "N"})
    ...
    def delete_filtergroup(self, groupid):
        ...
        self._db.query(CONFIGFILTERRULES).filter(
            CONFIGFILTERRULES.GROUP_ID == groupid
        ).delete()
        self._db.query(CONFIGFILTERGROUP).filter(
            CONFIGFILTERGROUP.ID == int(groupid)
        ).delete()
    ...
    def delete_filterrule(self, ruleid):
        ...
        """
        self._db.query(CONFIGFILTERRULES).filter(
            CONFIGFILTERRULES.ID == int(ruleid)
        ).delete()
    ...
    def get_userrss_tasks(self, tid=None):
        if tid:
            return (
                self._db.query(CONFIGUSERRSS)
                .filter(CONFIGUSERRSS.ID == int(tid))
                .all()
            )
        else:
            return (
                self._db.query(CONFIGUSERRSS)
                .order_by(CONFIGUSERRSS.STATE.desc())
                .all()
            )
    ...
    def update_userrss_task(self, item):
        if item.get("id") and self.get_userrss_tasks(item.get("id")):
            self._db.query(CONFIGUSERRSS).filter(
                CONFIGUSERRSS.ID == int(item.get("id"))
            ).update(
                {
                    "NAME": item.get("name"),
    ...
    def insert_userrss_mediainfos(self, tid=None, mediainfo=None):
        ...
        if not taskinfo:
            return
        mediainfos = (
            json.loads(taskinfo[0].MEDIAINFOS)
        ...
        self._db.query(CONFIGUSERRSS).filter(
            CONFIGUSERRSS.ID == int(tid)
        ).update({"MEDIAINFOS": json.dumps(mediainfos)})

    def get_userrss_parser(self, pid=None):
        if pid:
            return (
                self._db.query(CONFIGRSSPARSER)
                .filter(CONFIGRSSPARSER.ID == int(pid))
                .first()
            )
        else:
    ...
    def is_custom_words_existed(self, replaced=None, front=None, back=None):
        ...
        if replaced:
            count = (
                self._db.query(CUSTOMWORDS)
                .filter(CUSTOMWORDS.REPLACED == replaced)
                .count()
            )
        elif front and back:
            count = (
    ...
    def insert_custom_word_groups(
        self, title, year, gtype, tmdbid, season_count, note=None
    ):
        """
        增加自定义识别词组
        """
    ...
    def delete_custom_word_group(self, gid):
        ...
        if not gid:
            return
        self._db.query(CUSTOMWORDS).filter(
            CUSTOMWORDS.GROUP_ID == int(gid)
        ).delete()
        self._db.query(CUSTOMWORDGROUPS).filter(
            CUSTOMWORDGROUPS.ID == int(gid)
        ).delete()
    ...
    def get_custom_word_groups(self, gid=None, tmdbid=None, gtype=None):
        ...
        if gid:
            return (
                ...
                .filter(CUSTOMWORDGROUPS.ID == int(gid))
                .all()
            )
        if tmdbid and gtype:
            return (
                ...
                .filter(
                    ...
                    CUSTOMWORDGROUPS.TYPE == int(gtype),
                )
                .all()
            )
    ...
    def get_config_sync_paths(self, sid=None):
        ...
        if sid:
            return (
                self._db.query(CONFIGSYNCPATHS)
                .filter(CONFIGSYNCPATHS.ID == int(sid))
                .all()
            )
        return (
            self._db.query(CONFIGSYNCPATHS)
            .order_by(CONFIGSYNCPATHS.SOURCE)
            .all()
        )

    @DbPersist(_db)
    def check_config_sync_paths(
        self, sid=None, compatibility=None, rename=None, enabled=None
    ):
        """
        设置目录同步状态
        """
        if sid and rename is not None:
            ...
            self._db.query(CONFIGSYNCPATHS).filter(
                CONFIGSYNCPATHS.ID == int(sid)
            ).update({"COMPATIBILITY": int(compatibility)})
    ...
    def get_download_setting(self, sid=None):
        ...
        if sid:
            return (
                self._db.query(DOWNLOADSETTING)
                .filter(DOWNLOADSETTING.ID == int(sid))
                .all()
            )
    ...
    def get_message_client(self, cid=None):
        ...
        if cid:
            return (
                self._db.query(MESSAGECLIENT)
                .filter(MESSAGECLIENT.ID == int(cid))
                .all()
            )
    ...
    def check_message_client(
        self, cid=None, interactive=None, enabled=None, ctype=None
    ):
        """
        设置目录同步状态
        """
        if cid and interactive is not None:
            self._db.query(MESSAGECLIENT).filter(
                MESSAGECLIENT.ID == int(cid)
            ).update({"INTERACTIVE": int(interactive)})
    ...
    def get_torrent_remove_tasks(self, tid=None):
        ...
        if tid:
            return (
                self._db.query(TORRENTREMOVETASK)
                .filter(TORRENTREMOVETASK.ID == int(tid))
                .all()
            )
        return (
            self._db.query(TORRENTREMOVETASK)
            .order_by(TORRENTREMOVETASK.NAME)
            .all()
        )
    ...
    def update_downloader(
        ...
        """
        if did:
            self._db.query(DOWNLOADER).filter(
                DOWNLOADER.ID == int(did)
            ).update(
                {
                    "NAME": name,
                    "ENABLED": int(enabled),
    ...
    def check_downloader(
        ...
        if transfer is not None:
            self._db.query(DOWNLOADER).filter(
                DOWNLOADER.ID == int(did)
            ).update({"TRANSFER": int(transfer)})
        elif only_nastool is not None:
            self._db.query(DOWNLOADER).filter(
                DOWNLOADER.ID == int(did)
            ).update({"ONLY_NASTOOL": int(only_nastool)})
        elif match_path is not None:
            self._db.query(DOWNLOADER).filter(
                DOWNLOADER.ID == int(did)
            ).update({"MATCH_PATH": int(match_path)})
        elif enabled is not None:
            self._db.query(DOWNLOADER).filter(
                DOWNLOADER.ID == int(did)
            ).update({"ENABLED": int(enabled)})
    ...
    def get_indexer_statistics(self):
        ...
        return (
            self._db.query(
                ...
                func.sum(
                    case((INDEXERSTATISTICS.RESULT == "N", 1), else_=0)
                ).label("FAIL"),
                func.sum(
                    case((INDEXERSTATISTICS.RESULT == "Y", 1), else_=0)
                ).label("SUCCESS"),
    ...
    def get_plugin_history(self, plugin_id, key):
        ...
        if key:
            return (
                ...
                .filter(
                    PLUGINHISTORY.PLUGIN_ID == plugin_id,
                    PLUGINHISTORY.KEY == key,
                )
                .first()
            )
        else:
            return (
                self._db.query(PLUGINHISTORY)
                .filter(PLUGINHISTORY.PLUGIN_ID == plugin_id)
                .all()
            )
    ...
    def update_plugin_history(self, plugin_id, key, value):
        ...
        更新插件运行记录
        """
        self._db.query(PLUGINHISTORY).filter(
            PLUGINHISTORY.PLUGIN_ID == plugin_id, PLUGINHISTORY.KEY == key
        ).update({"VALUE": value})

    @DbPersist(_db)
    def delete_plugin_history(
        self, plugin_id: str, key=None, is_delete_all=False
    ):
        """
        删除插件运行记录
        """
