...
def web():
    ...
    SearchSource = (
        "douban"
        if Config().get_config("laboratory").get("use_douban_titles")
        else "tmdb"
    )
    CustomScriptCfg = SystemConfig().get(SystemConfigKey.CustomScript)
    __CooperationSites = current_user.get_authsites()
    exclude_sites = [
        "sharkpt",
        "hhclub",
        "piggo",
    ...
    CooperationSites = {
        k: v for k, v in __CooperationSites.items() if k not in exclude_sites
    }
...
def movie_rss():
    ...
    RuleGroups = {
        str(group["id"]): group["name"] for group in Filter().get_rule_groups()
    }
...
def sitelist():
    ...
    return render_template(
        "site/sitelist.html", Sites=IndexerSites, Count=len(IndexerSites)
    )
...
def resources():
    ...
    Results = (
        WebAction()
        .list_site_resources({"id": site_id, "page": page, "keyword": keyword})
        .get("data")
        or []
    )
    return render_template(
...
def recommend():
    ...
    return render_template(
        ...
        Filter=FilterKey,
        FilterConf=(
            ModuleConf.DISCOVER_FILTER_CONF.get(FilterKey) if FilterKey else {}
        ),
        Params=Params,
    )
...
def media_detail():
    ...
    return render_template(
        "discovery/mediainfo.html", TmdbId=TmdbId, Type=Type
    )
...
@login_required
def downloading():
    DispTorrents = WebAction().get_downloading().get("result") or []
    return render_template(
...
def statistics():
    ...
    # 近期上传下载各站点汇总
    # CurrentUpload, CurrentDownload, _, _, _ = (
    #     SiteUserInfo().get_pt_site_statistics_history(days=2)
    # )

    # 站点用户数据
    SiteUserStatistics = (
        WebAction().get_site_user_statistics({"encoding": "DICT"}).get("data")
    )
...
def history():
    ...
    PageRange = WebUtils.get_page_range(
...
def tmdbcache():
    ...
    page_range = WebUtils.get_page_range(
...
def customwords():
    ...
    return render_template(
        "setting/customwords.html", Groups=groups, GroupsCount=len(groups)
    )
...
def downloader():
    ...
    Categories = {
        x: WebAction().get_categories({"type": x}).get("category")
        for x in ["电影", "电视剧", "动漫"]
    }
    RmtModeDict = WebAction().get_rmt_modes()
    return render_template(
...
def library():
    return render_template(
        "setting/library.html", Config=Config().get_config()
    )
...
def rss_parser():
    RssParsers = RssChecker().get_userrss_parser()
    return render_template(
        "rss/rss_parser.html", RssParsers=RssParsers, Count=len(RssParsers)
    )
...
def plugin():
    ...
    return render_template(
        "setting/plugin.html", Plugins=Plugins, Count=len(Plugins)
    )
...
def wechat():
    ...
    if request.method == "GET":
        if not sVerifyMsgSig and not sVerifyTimeStamp and not sVerifyNonce:
            return (
                "NAStool微信交互服务正常！<br>微信回调配置步聚：<br>1、"
                "在微信企业应用接收消息设置页面生成Token和EncodingAESKey并填入设置->消息通知->微信对应项，"
                "打开微信交互开关。<br>2、保存并重启本工具，保存并重启本工具，"
                "保存并重启本工具。<br>3、"
                "在微信企业应用接收消息设置页面输入此地址：http(s)://IP:PORT/wechat（IP、"
                "PORT替换为本工具的外网访问地址及端口，"
                "需要有公网IP并做好端口转发，最好有域名）。"
            )
        ...
        try:
            ...
            if msg_type == "event" and event == "click":
                ...
                if conf.get("adminUser") and not any(
                    user_id == admin_user
                    for admin_user in str(conf.get("adminUser")).split(";")
                ):
                    Message().send_channel_msg(
                        channel=SearchType.WX,
                        title="用户无权限执行菜单命令",
                        user_id=user_id,
                    )
                ...
                if event_key:
                    ...
                    if len(keys) > 2:
                        content = ModuleConf.WECHAT_MENU.get(keys[2]) or ""
            elif msg_type == "text":
                # 文本消息
...
def plex_webhook():
    ...
    # 事件类型
    event_match = request_json.get("event") in [
        "media.play",
        "media.stop",
        "library.new",
    ]
    ...
    if event_match and type_match and not is_live:
        ...
        ThreadHelper().start_thread(
            MediaServer().webhook_message_handler,
...
def telegram():
    ...
    if not SecurityHelper().check_telegram_ip(request.remote_addr):
        log.error(
            "收到来自 %s 的非法Telegram消息：%s"
            % (request.remote_addr, msg_json)
        )
        return "不允许的IP地址请求"
    if msg_json:
        ...
        if text:
            log.info(
                f"收到Telegram消息：userid={user_id}, username={user_name},"
                f" text={text}"
            )
            # 检查权限
            if text.startswith("/"):
                if (
                    str(user_id)
                    not in interactive_client.get("client").get_admin()
                ):
                    Message().send_channel_msg(
                        channel=SearchType.TG,
                ...
                if (
                    not str(user_id)
                    in interactive_client.get("client").get_users()
                ):
                    Message().send_channel_msg(
                        channel=SearchType.TG,
...
def synology():
    ...
    if msg_data:
        ...
        if not interactive_client.get("client").check_token(token):
            log.error(
                "收到来自 %s 的非法Synology Chat消息：token校验不通过！"
                % request.remote_addr
            )
            return "token校验不通过"
        ...
        if text:
            log.info(
                f"收到Synology Chat消息：userid={user_id},"
                f" username={user_name}, text={text}"
            )
            WebAction().handle_message_job(
                msg=text,
...
def ical():
    ...
    response.headers["Content-Disposition"] = (
        "attachment; filename=nastool.ics"
    )
    return response
...
def Img():
    ...
    # 获取图片数据
    response = Response(WebUtils.request_cache(url), mimetype="image/jpeg")
        ...
        while True:
            with LoggingLock:
                ...
                if log.LOG_INDEX > 0:
                    ...
                    if _source:
                        logs = [
                            lg for lg in logs if lg.get("source") == _source
                        ]
                else:
                    logs = []
                time.sleep(1)
...
def stream_progress():
    ...
    return Response(
        __progress(request.args.get("type")), mimetype="text/event-stream"
    )
...
def message_handler(ws):
    ...
    while True:
        ...
        if msgbody.get("text"):
            ...
            # 拉取消息
            system_msg = WebAction().get_system_message(
                lst_time=msgbody.get("lst_time")
            )
            ...
            for message in list(reversed(messages)):
                ...
                ret_messages.append(
                    {
                        "level": (
                            "bg-red" if message.get("level") == "ERROR" else ""
                        ),
            ...
            ws.send(
                (json.dumps({"lst_time": lst_time, "message": ret_messages}))
            )
