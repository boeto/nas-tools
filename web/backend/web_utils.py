...
class WebUtils:
    ...
    def get_location(ip):
        ...
        url = (
            ...
            "api.php?co=&resource_id=6006&t=1529895387942&ie=utf8"
            "&oe=gbk&cb=op_aladdin_callback&format=json&tn=baidu&"
            "cb=jQuery110203920624944751099_1529894588086&_"
    ...
    def get_latest_version():
        ...
        """
        # try:
        #     releases_update_only = (
        #         Config().get_config("app").get("releases_update_only")
        #     )
        #     proxies = Config().get_proxies()
        #     version_res = RequestUtils(__proxies).get_res(
        #         "https://nastool.cn/"
        #         f"{quote(WebUtils.get_current_version())}/update"
        #     )
        #     if version_res:
    ...
    def get_mediainfo_from_id(mtype, mediaid, wait=False):
        ...
        if str(mediaid).startswith("DB:"):
            # 豆瓣
            doubanid = mediaid[3:]
            info = DouBan().get_douban_detail(
                doubanid=doubanid, mtype=mtype, wait=wait
            )
            if not info:
                return None
