...
const search_source_icon = {
  tmdb: html` <!-- http://tabler-icons.io/i/square-letter-t -->
    <svg
      xmlns="http://www.w3.org/2000/svg"
      class="icon icon-tabler icon-tabler-square-letter-t text-blue"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      stroke-width="2"
  ...
  douban: html`
    ...
    <svg
      xmlns="http://www.w3.org/2000/svg"
      class="icon icon-tabler icon-tabler-circle-letter-d text-green"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      stroke-width="2"
  ...
  person: html`
    ...
    <svg
      xmlns="http://www.w3.org/2000/svg"
      class="icon icon-tabler icon-tabler-square-rounded-letter-p text-purple"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      stroke-width="2"
...
export class LayoutSearchbar extends CustomElement {
  ...
  firstUpdated() {
    this._search_source =
      localStorage.getItem("SearchSource") ?? this.layout_search_source;
    // 当前状态：是否模糊
    let blur = false;
      ...
      <div class="container-fluid nav-search-bar">
        <div class="d-flex flex-row flex-grow-1 align-items-center py-1">
          ...
          <div class="input-group input-group-flat mx-2">
            ...
            <input
              type="text"
              class="home_search_bar form-control form-control-rounded"
              placeholder=${this._search_source === "person"
                ? "搜索人物"
                : "搜索电影、电视剧"}
              autocomplete="new-password"
              ?readonly=${this.layout_search == 0}
              @keypress=${(e) => {
                if (e.which === 13 && this.input.value) {
                  if (this._search_source === "person") {
                    navmenu(
                      "discovery_person?&type=ALL&title=演员搜索&subtitle=" +
                        this.input.value +
                        "&keyword=" +
                        this.input.value
                    );
                    ...
                    navmenu(
                      "recommend?type=SEARCH&title=搜索结果&subtitle=" +
                        this.input.value +
                        "&keyword=" +
                        this.input.value +
                        "&source=" +
            ...
            <span class="input-group-text form-control-rounded">
              <a
                href="${this.layout_search > 0
                  ? "javascript:show_search_advanced_modal()"
                  : "javascript:void(0)"}"
                class="link-secondary"
              >
          ...
          <div class="nav-item dropdown me-2">
            <a
              href="#"
              class="nav-link d-flex lh-1 text-reset ms-1 p-0"
            ...
            <div class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
              <a
                class="dropdown-item hide-theme-dark"
                href="?theme=dark"
                role="button"
                >暗黑风格</a
              >
              <a
                class="dropdown-item hide-theme-light"
                href="?theme=light"
                role="button"
                >明亮风格</a
              >
              ...
              ${this.layout_useradmin === "1"
                ? html`
                    ...
                    <a
                      class="dropdown-item"
                      href="javascript:show_logging_modal()"
                      role="button"
                      >实时日志</a
                    >
                    <div class="dropdown-divider"></div>
              ...
              <a href="javascript:logout()" class="dropdown-item">
                注销
                <span class="text-muted mx-3">${this.layout_username}</span>
              </a>
              <div class="dropdown-divider"></div>
              <a
