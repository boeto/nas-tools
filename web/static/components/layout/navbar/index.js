import { LayoutNavbarButton } from "./button.js";
export { LayoutNavbarButton };
...
export class LayoutNavbar extends CustomElement {
  ...
  constructor() {
    ...
    this._update_url = "https://hub.docker.com/r/boeto/nas-tools/tags";
  ...
  _init_page() {
    ...
    if (this.layout_gopage) {
      ...
      if (page) {
        ...
        // 打开第一个页面
        const page =
          this.navbar_list[0].page ?? this.navbar_list[0].list[0].page;
      ...
      if (ret.code === 0) {
        ...
        switch (compareVersion(ret.version, this.layout_appversion)) {
          case 1:
            ...
            break;
          case 2:
            url = "https://github.com/boeto/nas-tools/commits/master";
            break;
        }
        if (url) {
  ...
  render() {
    return html`
      <div class="container-fluid">
        <div
          class="offcanvas offcanvas-start d-flex lit-navbar-canvas shadow"
          tabindex="-1"
          id="litLayoutNavbar"
        >
          <div class="d-flex flex-row flex-grow-1 lit-navbar-hide-scrollbar">
            <div class="d-flex flex-column flex-grow-1">
              <h1 class="mt-3" style="text-align:center;">
                <img
                  src="../static/img/logo/logo-blue.png"
                  alt="NAStool"
                  class="lit-navbar-logo"
                />
              </h1>
              <div class="accordion px-2 py-2 flex-grow-1">
                ${this.navbar_list.map(
                  (item, index) => html`
                    ${item.list?.length > 0
                      ? html` <button
                            class="accordion-button lit-navbar-accordion-button collapsed ps-2 pe-1 py-2"
                            style="font-size:1.1rem;"
                            data-bs-toggle="collapse"
                            data-bs-target="#lit-navbar-collapse-${index}"
                            aria-expanded="false"
                          >
                            ${item.also ?? item.name}
                          </button>
                          <div
                            ...
                            id="lit-navbar-collapse-${index}"
                          >
                            ${item.list.map((drop) =>
                              this._render_page_item(drop, true)
                            )}
                          </div>`
              ...
              <div class="d-flex align-items-end">
                ${this.layout_useradmin === "1"
                  ? html`
                      ${this.layout_userlevel > 1
                        ? html`
                            <!-- 升级提示 -->
                            <span
                              class="d-flex flex-grow-1 justify-content-center border rounded-3 m-3 p-2 ${this
                                ._is_update
                                ? "bg-yellow"
                                : ""}"
                            >
                              <a
                                href=${this._update_url}
                                class="${this._is_update
                                  ? "text-yellow-fg"
                                  : "text-muted"}"
                                target="_blank"
                                rel="noreferrer"
                              >
                                <strong>
                                  <svg
                                    ...
                                    class="icon icon-tabler icon-tabler-brand-docker {{ class }}"
                                    width="24"
                                    height="24"
                                    viewBox="0 0 24 24"
                                    stroke-width="2"
                                  ...
                                  ${!this._is_update
                                    ? this.layout_appversion
                                    : html`<del
                                        >${this.layout_appversion}</del
                                      >`}
                                </strong>
                              </a>
                              ${this._is_update
                                ? html` <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    class="cursor-pointer icon icon-tabler icon-tabler-arrow-big-up-lines-filled ms-2 text-red"
                                    width="24"
                                    height="24"
                                    viewBox="0 0 24 24"
                                    stroke-width="2"
                        ...
                        : html`
                            <!-- 用户认证 -->
                            <button
                              class="btn btn-outline-secondary w-100 m-3 p-2"
                              onclick="show_user_auth_modal()"
                            >
                              <strong>
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
  ...
  _render_page_item(item, child) {
    return html` <a
      class="nav-link lit-navbar-accordion-item${this._active_name === item.page
        ? "-active"
        : ""} my-1 p-2 ${child ? "ps-3" : "lit-navbar-accordion-button"}"
      href="#${item.page}"
    ...
    >
      <span
        class="nav-link-icon"
        ?hidden=${!child}
        style="color:var(--tblr-body-color);"
      >
        ${item.icon ? unsafeHTML(item.icon) : nothing}
      </span>
      <span class="nav-link-title"> ${item.also ?? item.name} </span>
    </a>`;
  }
}
