...
def check_config():
    ...
    if Config().get_config("app"):
        ...
        if not login_user or not login_password:
            log.warn(
                "【Config】WEB管理用户或密码未设置，将使用默认用户：admin，"
                "密码：password"
            )
        else:
        ...
        if not ssl_cert or not ssl_key:
            log.info(
                f"未启用https，请使用 http://IP:{str(web_port)} 访问管理页面"
            )
        else:
            ...
            log.info(
                f"已启用https，请使用 https://IP:{str(web_port)} 访问管理页面"
            )
    else:
...
def update_config():
    ...
    try:
        ...
        if subtitle:
            if subtitle.get("server") == "opensubtitles":
                PluginManager().save_plugin_config(
                    pid="OpenSubtitles",
                    conf={
                        "enable": subtitle.get("opensubtitles", {}).get(
                            "enable"
                        )
                    },
                )
    ...
    # 自定义制作组/字幕组兼容旧配置
    try:
        custom_release_groups = (Config().get_config("laboratory") or {}).get(
            "release_groups"
        )
        if custom_release_groups:
            PluginManager().save_plugin_config(
                pid="CustomReleaseGroups",
                conf={"release_groups": custom_release_groups},
            )
            # 删除旧配置
    ...
    try:
        ...
        if douban:
            _enable = (
                True
                if douban.get("users")
                and douban.get("interval")
                and douban.get("types")
                else False
            )
            PluginManager().save_plugin_config(
                pid="DoubanSync",
    ...
    try:
        ...
        # 保存
        if scraper_conf:
            SystemConfig().set(SystemConfigKey.UserScraperConf, scraper_conf)
    except Exception as e:
        ExceptionUtils.exception_traceback(e)
    ...
    try:
        ...
        if indexer_sites:
            SystemConfig().set(SystemConfigKey.UserIndexerSites, indexer_sites)
    ...
    # 存量插件安装情况统计
    try:
        plugin_report_state = SystemConfig().get(
            SystemConfigKey.UserInstalledPluginsReport
        )
        installed_plugins = SystemConfig().get(
        ...
        if not plugin_report_state and installed_plugins:
            ...
            if ret:
                SystemConfig().set(
                    SystemConfigKey.UserInstalledPluginsReport, "1"
                )
    except Exception as e:
        ExceptionUtils.exception_traceback(e)
...
class ConfigMonitor(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if file_name == "config.yaml" and not ConfigLoadCache.get(src_path):
            ...
            log.warn(
                f"【System】进程 {os.getpid()} 检测到系统配置文件已修改，正在重新加载..."
            )
            time.sleep(1)
            # 重新加载配置
            Config().init_config()
...
def start_config_monitor():
    ...
    # 配置文件监听
    _observer.schedule(
        ConfigMonitor(), path=Config().get_config_path(), recursive=False
    )
